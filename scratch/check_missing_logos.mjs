...
async function checkMore() {
  const { data, error } = await supabase.from('developers').select('name, logo_url')
  if (error) return console.error(error)
  
  const missing = data.filter(d => !d.logo_url)
  console.log('Missing logos:', missing.map(m => m.name))
}
