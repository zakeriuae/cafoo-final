...
async function setup() {
  // Disable SSL verification for this temporary setup script
  ...
  const sql = `
    ...
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Auth upload media' AND tablename = 'objects' AND schemaname = 'storage') THEN
    ...
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Auth update media' AND tablename = 'objects' AND schemaname = 'storage') THEN
    ...
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Auth delete media' AND tablename = 'objects' AND schemaname = 'storage') THEN
