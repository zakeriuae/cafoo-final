...
async function fixOpusImage() {
  const opusImage = 'https://images.unsplash.com/photo-1614605242014-22d76f069b1b?auto=format&fit=crop&q=80&w=1200' // Futuristic Dubai building
  
  const { error } = await supabase
    .from('towers')
    .update({ image_url: opusImage })
    .ilike('name', 'THE OPUS')

  if (error) console.error('Error fixing Opus image:', error)
  else console.log('✅ Successfully fixed THE OPUS image.')
}
