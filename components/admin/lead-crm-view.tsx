...
import { addLeadMessage, reassignLead } from '@/app/(admin)/admin/(dashboard)/leads/actions'
...
export function LeadCRMView({ lead, messages = [], agents = [] }: LeadCRMViewProps) {
  ...
  const handleReassign = async (agentId: string) => {
    const res = await reassignLead(lead.id, agentId)
    if (res.success) {
            ...
            <CardContent className="p-6">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="w-full h-auto p-2 justify-start hover:bg-slate-50 rounded-xl transition-all border border-slate-100">
                    {lead.agent ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <p className="font-bold text-slate-900 text-sm">{lead.agent.name}</p>
                          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Assigned Consultant</p>
                        </div>
                      </div>
                    ) : (
                      <div className="flex items-center gap-3 text-slate-400">
                        ...
                        <span className="text-sm font-medium italic">Click to Assign Agent</span>
                      </div>
                    )}
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="start" className="w-64 max-h-80 overflow-y-auto rounded-xl p-2">
                  <DropdownMenuLabel className="text-[10px] uppercase font-bold text-slate-400 px-2 py-1.5">Select Agent</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => handleReassign('none')} className="rounded-lg cursor-pointer">
                    <span className="text-xs font-bold text-red-500">Unassign All</span>
                  </DropdownMenuItem>
                  {agents.map((agent) => (
                    <DropdownMenuItem 
                      key={agent.id} 
            ...
            <CardContent className="p-6">
              {lead.property ? (
                ...
                <div className="text-center py-6 border-2 border-dashed border-slate-100 rounded-xl">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">No Linked Asset</p>
                </div>
              )}
            </CardContent>
          </Card>
                   ...
                   if (isAction) {
                     return (
                       <div key={msg.id} className="flex justify-center my-4">
                         <div className="bg-white/60 backdrop-blur-sm border border-slate-200/50 rounded-full px-4 py-1 flex items-center gap-2 shadow-sm">
                           <div className="p-1 rounded-full bg-slate-100">
                             {msg.metadata?.source === 'like' ? <Heart className="w-3 h-3 text-red-500 fill-current" /> :
                              msg.metadata?.source === 'call' ? <Phone className="w-3 h-3 text-blue-500" /> :
                              msg.metadata?.source === 'whatsapp' ? <MessageSquare className="w-3 h-3 text-green-500" /> :
                              <Clock className="w-3 h-3 text-slate-400" />}
                           </div>
                           <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">
                             {msg.content}
                           </span>
                           <span className="text-[9px] text-slate-300 ml-2">
                   ...
                   return (
                     <div key={msg.id} className={cn("flex gap-3", "justify-end")}>
                       <div className={cn(
                         ...
                         {/* Bubble tail */}
                         <div className="absolute top-0 -right-2 w-3 h-3 bg-[#E1FEC6] [clip-path:polygon(0%_0%,100%_0%,0%_100%)]" />
                       </div>
                       <Avatar className="w-8 h-8 border border-white shadow-sm self-start">
